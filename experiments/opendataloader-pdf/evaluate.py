#!/usr/bin/env python3
"""
evaluate.py — Score opendataloader-pdf HTML outputs against ground-truth source HTML.

Scoring rubric (same as other experiments for apples-to-apples comparison):
...
def evaluate_fixture(fixture: str) -> dict | None:
    ...
    if not converted_html_path.exists():
        print(f"[eval] SKIP {fixture} — no converted HTML (run convert.py first)")
        return None
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        fixture_names = sorted(
            d.name for d in Path(FIXTURES_DIR).iterdir()
